...
public class ContactsAction extends ContextBaseAction {
	...
	public final ActionForward frontPage(final ActionMapping mapping,
	...
	public final ActionForward createCustomEmail(final ActionMapping mapping,
			final ActionForm form, final HttpServletRequest request,
			final HttpServletResponse response) {
		...
		EmailAddress.createNewEmailAddress(emailBean.getValue(), person, false,
	...
	public final ActionForward modifyContacts(final ActionMapping mapping,
			final ActionForm form, final HttpServletRequest request,
			final HttpServletResponse response) {
		...
		List<PersistentGroup> existingVisbilityGroups = ContactsConfigurator
				.getInstance().getVisibilityGroups();
		/*
		...
		PhysicalAddress.createNewPhysicalAddress(Country.getPortugal(), "none",
				person, true, PartyContactType.PERSONAL).setVisibleTo(
				existingVisbilityGroups);
	...
	public final ActionForward configContactsModule(
			final ActionMapping mapping, final ActionForm form,
			final HttpServletRequest request, final HttpServletResponse response) {

		request.setAttribute("contactsConfiguration",
				ContactsConfigurator.getInstance());

		GroupsSelectorBean groupsSelectorBean = new GroupsSelectorBean();

		groupsSelectorBean.setGroups(ContactsConfigurator.getInstance()
				.getVisibilityGroups());
		...
		usersWithSuperEditorRoleBean
				.setUsers(new ArrayList<User>(
						((PersistentGroup) Role
								.getRole(ContactsConfigurator.getInstance()
										.getContactsRoles().MODULE_CONTACTS_DOMAIN_CONTACTSEDITOR))
								.getMembers()));

		request.setAttribute("usersWithSuperEditorRoleBean",
				usersWithSuperEditorRoleBean);

		return forward(request, "/contacts/configure/configureModule.jsp");
	}

	public final ActionForward editAlias(final ActionMapping mapping,
			final ActionForm form, final HttpServletRequest request,
			final HttpServletResponse response) {
		final PersistentGroup persistentGroup = AbstractDomainObject
				.fromExternalId(request.getParameter("groupId"));

		GroupAliasBean groupAliasBean = new GroupAliasBean();
		groupAliasBean.setGroupToEdit(persistentGroup);
		groupAliasBean
				.setAlias(persistentGroup.getGroupAlias().getGroupAlias());
		request.setAttribute("groupAliasBean", groupAliasBean);

		return forward(request,
				"/contacts/configure/setPersistentGroupAlias.jsp");

	}

	public final ActionForward modifyGroupAlias(final ActionMapping mapping,
			final ActionForm form, final HttpServletRequest request,
			final HttpServletResponse response) {
		GroupAliasBean groupAliasBean = getRenderedObject("myorg.modules.contacts.edit.setGroupAliasBean");
		GroupAlias newAlias = GroupAlias.create(
				groupAliasBean.getGroupToEdit(), groupAliasBean.getAlias());

		return configContactsModule(mapping, form, request, response);

	}

	public final ActionForward setVisibilityGroups(final ActionMapping mapping,
			final ActionForm form, final HttpServletRequest request,
			final HttpServletResponse response) {
		GroupsSelectorBean groupsSelectorBean = getRenderedObject("groupsSelectorBean");
		ContactsConfigurator.getInstance().setVisibilityGroups(
				groupsSelectorBean.getGroups());
	...
	public final ActionForward assignSuperEditorRoleToGroup(
			final ActionMapping mapping, final ActionForm form,
			final HttpServletRequest request, final HttpServletResponse response) {
		...
		for (PersistentGroup group : groupsSelectorBean.getGroups()) {
			ContactsConfigurator.getInstance()
					.assignSuperEditorToPersonsCurrentlyIn(group);
		}

		return configContactsModule(mapping, form, request, response);
	}

	public final ActionForward assignSuperEditorRoleToPerson(
			final ActionMapping mapping, final ActionForm form,
			final HttpServletRequest request, final HttpServletResponse response) {
		PersonsBean personsBean = getRenderedObject("personsBean");

		ContactsConfigurator.getInstance().assignSuperEditorRole(
				personsBean.getPerson().getUser());

		return configContactsModule(mapping, form, request, response);
	}

	public final ActionForward removeUserFromSuperEditorRole(
			final ActionMapping mapping, final ActionForm form,
			final HttpServletRequest request, final HttpServletResponse response) {
		final User user = AbstractDomainObject.fromExternalId(request
				.getParameter("userId"));
	...
	public final ActionForward searchContacts(final ActionMapping mapping,
			final ActionForm form, final HttpServletRequest request,
			final HttpServletResponse response) {
		final PersistentGroup persistentGroup = AbstractDomainObject
				.fromExternalId(request.getParameter("groupId"));
	 ...
	 *         the search
	 */
	private CollectionPager<Person> searchPerson(PersonsBean personSearchBean,
			HttpServletRequest request, boolean useBean) {
		...
		if (useBean) {
			...
			String searchPhoneTypeString = request
					.getParameter("searchPhoneType");
			searchPhoneType = (searchPhoneTypeString != null) ? PhoneType
					.valueOf(searchPhoneTypeString) : null;
			String searchWebAddress = request.getParameter("searchWebAddress");

			Boolean resultsByDetails = (request
					.getParameter("resultsByDetails") != null) ? Boolean
					.getBoolean(request.getParameter("resultsByDetails"))
					: null;
		...
		ArrayList<Person> results = new ArrayList(ContactsConfigurator
				.getInstance().getPersonsByDetails(UserView.getCurrentUser(),
						personSearchBean.getSearchName(),
						personSearchBean.getSearchUsername(),
						personSearchBean.getSearchPhone(),
						personSearchBean.getSearchPhoneType(),
						personSearchBean.getSearchAddress(),
						personSearchBean.getSearchWebAddress(),
						personSearchBean.getSearchEmail()));

		return new CollectionPager<Person>(results,
				ContactsConfigurator.SEARCH_MAXELEMENTS_PER_PAGE);

	}

	public final ActionForward searchPersonsByDetails(
			final ActionMapping mapping, final ActionForm form,
			final HttpServletRequest request, final HttpServletResponse response) {
		...
		if (personSearchBean == null) {
			...
			personSearchBean.setSearchResult(searchPerson(personSearchBean,
					request, false).getPage(personSearchBean.getPageNumber()));
		} else {
			personSearchBean.setSearchResult(searchPerson(personSearchBean,
					request, true).getPage(personSearchBean.getPageNumber()));
		}
	...
	public final ActionForward searchPersonsByName(final ActionMapping mapping,
			final ActionForm form, final HttpServletRequest request,
			final HttpServletResponse response) {
		...
		if (personSearchBean.getPerson() == null) {
			...
			results = new CollectionPager<Person>(Collections.singleton(personSearchBean.getPerson()), ContactsConfigurator.SEARCH_MAXELEMENTS_PER_PAGE);
			personSearchBean.setPerson(null);
		}
		personSearchBean.setSearchResult(results.getPage(personSearchBean.getPageNumber()));
	...
	public final ActionForward editContacts(final ActionMapping mapping,
			final ActionForm form, final HttpServletRequest request,
			final HttpServletResponse response) {
		final PersistentGroup persistentGroup = AbstractDomainObject
				.fromExternalId(request.getParameter("groupId"));
		...
		if (request.getParameter("personEId") != null) {
			...
			person = AbstractDomainObject.fromExternalId(request
					.getParameter("personEId"));
			if (person == null)
				throw new DomainException(
						"manage.contacts.edit.error.noPerson",
						"resources.ContactsResources");
			if (!ContactsConfigurator.getInstance().isAllowedToEditContacts(
					UserView.getCurrentUser(), person))
				throw new DomainException(
						"manage.contacts.edit.denied.forPerson",
						"resources.ContactsResources", person.getName());
		...
		if (person != null) {
			...
			TreeSet<PartyContact> sortedContacts = new TreeSet<PartyContact>(
					chain);
			...
			ArrayList<PersistentGroup> visibilityGroups = new ArrayList<PersistentGroup>(
					ContactsConfigurator.getInstance().getVisibilityGroups());
	...
	public final ActionForward deletePartyContact(final ActionMapping mapping,
			final ActionForm form, final HttpServletRequest request,
			final HttpServletResponse response) {
		PartyContact contactToDelete = getDomainObject(request,
				"partyContactOid");
		// check if the contact can be edited or not by the user
		if (!contactToDelete.isEditableBy(UserView.getCurrentUser()))
			throw new DomainException("manage.contacts.edit.denied", UserView
		...
		request.setAttribute("personEId", contactToDelete.getOwner()
				.getExternalId());
	...
	public final ActionForward choosePhoneRegexpPostBack(
			final ActionMapping mapping, final ActionForm form,
			final HttpServletRequest request, final HttpServletResponse response) {
	...
	public final ActionForward editPartyContact(final ActionMapping mapping,
			final ActionForm form, final HttpServletRequest request,
			final HttpServletResponse response) {
		...
		ContactToEditBean contactToEditBean = getAttribute(request,
				"contactToEditBean");
		...
		visibilityGroupsConcededBean.setGroups(contactToEdit
				.getVisibilityGroups());
		request.setAttribute("visibilityGroupsConcededBean",
				visibilityGroupsConcededBean);
		// get all of the visibility groups
		...
		if (contactToEdit instanceof Phone) {
			...
			if (phoneToEdit.getPhoneType().equals(PhoneType.VOIP_SIP))
				return forward(request,
						"/contacts/editContact/editVoipPhoneContact.jsp");
			else if (phoneToEdit.getPhoneType().equals(PhoneType.EXTENSION))
				return forward(request,
						"/contacts/editContact/editExtensionPhoneContact.jsp");
			else
				return forward(request,
						"/contacts/editContact/editRegularPhoneContact.jsp");

		} else if (contactToEdit instanceof EmailAddress) {
			return forward(request,
					"/contacts/editContact/editOneFieldContact.jsp");
		} else if (contactToEdit instanceof PhysicalAddress) {
			return forward(request,
					"/contacts/editContact/editPhysicalAddressContact.jsp");
	...
	public final ActionForward applyPartyContactEdit(
			final ActionMapping mapping, final ActionForm form,
			final HttpServletRequest request, final HttpServletResponse response) {
		...
		if (contactToEdit instanceof Phone)
			((Phone) contactToEdit).changePhoneType(contactToEditBean
					.getPhoneType());
	...
	public final ActionForward createPartyContact(final ActionMapping mapping,
			final ActionForm form, final HttpServletRequest request,
			final HttpServletResponse response) {
	...
	public final ActionForward chooseKindOfContactPostBack(
			final ActionMapping mapping, final ActionForm form,
			final HttpServletRequest request, final HttpServletResponse response) {
	...
	public final ActionForward chooseGeographicLevelPostBack(
			final ActionMapping mapping, final ActionForm form,
			final HttpServletRequest request, final HttpServletResponse response) {
		...
		if (contactToCreateBean.getPhysicalAddressBean().getCountry() != null) {
			Country country = contactToCreateBean.getPhysicalAddressBean()
					.getCountry();
			HashMap<String, CountrySubdivision> geographicLevels = contactToCreateBean
					.getPhysicalAddressBean().getGeographicLevels();
			...
			if (!createGeographicLocations) {
				...
				contactToCreateBean.getPhysicalAddressBean().setAddressBean(
						AddressBeanFactory.createAddressBean(country));
				...
				if (geographicLevels.size() == 0) { // if we are on the first
					...
					geographicLevels.put(
							country.getSubdivisionLevelName(
									new Integer(FIRST_LEVEL)).getContent(),
							null);
					...
					ArrayList<CountrySubdivision> geographicLevelsOrdered = PhysicalAddressBean
							.getSubdivisionsOrderedArrayList(geographicLevels
									.values());

					int nextLevel = geographicLevelsOrdered
							.get(geographicLevelsOrdered.size() - 1).getLevel()
							.intValue() + 1;

					CountrySubdivisionLevelName nextSubdivision = country
							.getCountrySubdivisionLevel(nextLevel);
					if (nextSubdivision != null) {
						contactToCreateBean.getPhysicalAddressBean()
								.setAddressBean(null);
						geographicLevels.put(nextSubdivision.getName()
								.getContent(), null);
					} else
						contactToCreateBean.getPhysicalAddressBean()
								.setAddressBean(
										AddressBeanFactory
												.createAddressBean(country));

				}
			}
			// TODO take care of the labels that might exist

			contactToCreateBean.getPhysicalAddressBean().setGeographicLevels(
					geographicLevels);
		}
