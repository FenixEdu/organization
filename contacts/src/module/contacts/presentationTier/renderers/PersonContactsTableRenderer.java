...
public class PersonContactsTableRenderer extends OutputRenderer {
	...
	return new Layout() {
		...
		for (MetaObject contactMeta : contacts) {
		    ...
		    HtmlTableRow row = rowsByHeadersMap.get(RenderUtils.getResourceString(getBundle(), contactMeta.getObject()
			    .getClass().getName()));
		    if (row == null) {
			...
			// displayed correctly
			String headerLabel = RenderUtils.getResourceString(getBundle(), contactMeta.getObject().getClass()
				.getName());
